...
def burn_side(request):
    if request.GET.get('n') and request.GET.get('k'):
        try:
            agent = BurnSide(
                int(request.GET.get('n')), float(request.GET.get('k')))
        except ValueError:
...
def view_questions(request):
    ...
    return render(request, "main_site/view_question.html", {
...
def super_view_questions(request):
    question_list = Question.objects.all().order_by('order')
    return render(request, "main_site/super_view_question.html", {
        "questions": question_list
        })
...
def response_questions(request, id):
    ...
    if request.method == "POST":
        ...
        return render(request, "main_site/response_question.html", {
            "question": question
            })
...
def view_slide(request):
    ...
    return render(request, "main_site/view_slides.html", {
        'slides': text_slide
        })
